# app.py — webhook app (FastAPI). Minimal evaluate_symbol stub included.
...
# Minimal evaluate_symbol stub — replace with your real function later
def evaluate_symbol(symbol: str):
    ...
    return {
        ...
        "survivability": {
            "1m": round(score, 3),
            "2m": round(max(0.0, score * 0.7), 3),
            "3m": round(max(0.0, score * 0.5), 3),
        },
    ...
    if isinstance(text, str) and text.strip().lower().startswith("/scan"):
        ...
        try:
            ...
            if result:
                ...
                text_out += "Strategies: " + ", ".join([f"{k}:{v}" for k, v in strategies.items()])
            else:
